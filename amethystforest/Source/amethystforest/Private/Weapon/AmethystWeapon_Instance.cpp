...
void AAmethystWeapon_Instance::FireWeapon()
{
...
bool AAmethystWeapon_Instance::ServerNotifyHit_Validate(const FHitResult Impact, FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
    return true;
}

void AAmethystWeapon_Instance::ServerNotifyHit_Implementation(const FHitResult Impact, FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
...
bool AAmethystWeapon_Instance::ServerNotifyMiss_Validate(FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
    return true;
}

void AAmethystWeapon_Instance::ServerNotifyMiss_Implementation(FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
    const FVector Origin = GetMuzzleLocation();
...
void AAmethystWeapon_Instance::ProcessInstantHit(const FHitResult& Impact, const FVector& Origin, const FVector& ShootDir, int32 RandomSeed, float ReticleSpread)
{
...
void AAmethystWeapon_Instance::ProcessInstantHit_Confirmed(const FHitResult& Impact, const FVector& Origin, const FVector& ShootDir, int32 RandomSeed, float ReticleSpread)
{
    // handle damage
...
bool AAmethystWeapon_Instance::ShouldDealDamage(AActor* TestActor) const
{
...
void AAmethystWeapon_Instance::DealDamage(const FHitResult& Impact, const FVector& ShootDir)
{
...
void AAmethystWeapon_Instance::OnBurstFinished()
{
...
float AAmethystWeapon_Instance::GetCurrentSpread() const
{
...
void AAmethystWeapon_Instance::SimulateInstantHit(const FVector& ShotOrigin, int32 RandomSeed, float ReticleSpread)
{
    FRandomStream WeaponRandomStream(RandomSeed);
...
void AAmethystWeapon_Instance::SpawnImpactEffects(const FHitResult& Impact)
{
    if (ImpactTemplate && Impact.bBlockingHit)
    {
        ...
        AAmethystImpactEffect* EffectActor = GetWorld()->SpawnActorDeferred<AAmethystImpactEffect>(ImpactTemplate, Impact.ImpactPoint, Impact.ImpactNormal.Rotation());
        if (EffectActor)
        {
...
void AAmethystWeapon_Instance::SpawnTrailEffect(const FVector& EndPoint)
{
    if (TrailFX)
    {
        const FVector Origin = GetMuzzleLocation();
...
void AAmethystWeapon_Instance::GetLifetimeReplicatedProps( TArray< FLifetimeProperty > & OutLifetimeProps ) const
{
    Super::GetLifetimeReplicatedProps( OutLifetimeProps );
    
    DOREPLIFETIME_CONDITION( AAmethystWeapon_Instance, HitNotify, COND_SkipOwner );
}
